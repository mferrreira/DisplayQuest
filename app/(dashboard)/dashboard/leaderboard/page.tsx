  ...
  return (
    <div className="flex min-h-screen flex-col">
      <main className="container mx-auto flex-1 space-y-8 p-4 md:p-6">
              {/* dark-mode:ok (hero já escuro por design nos 2 temas) */}
            ...
            <div className="space-y-3">
              ...
              <div>
                <h1 className="text-3xl font-black tracking-tight md:text-4xl">Quadro de Lideranca</h1>
                                {/* dark-mode:ok (texto sobre hero escuro nos 2 temas) */}
                <p className="max-w-2xl text-sm text-white/70 md:text-base">
                  Veja o podio do laboratorio, acompanhe quem mais pontuou e navegue pelos perfis para entender badges, tarefas concluidas e participacao em projetos.
                            ...
                            {/* dark-mode:ok (card translúcido sobre hero escuro) */}
        ...
        <section className="grid gap-5 lg:grid-cols-3">
          {podiumUsers.map((podiumUser, index) => {
            ...
            const emphasis =
              position === 1
                ? "lg:col-span-1 lg:scale-[1.03] border-yellow-200 dark:border-yellow-500/40 bg-gradient-to-b from-yellow-50 to-amber-100 dark:from-yellow-500/10 dark:to-amber-500/10 dark:shadow-none"
                : position === 2
                  ? "border-slate-200 dark:border-slate-500/40 bg-gradient-to-b from-slate-50 to-slate-100 dark:from-slate-500/10 dark:to-slate-400/10"
                  : "border-orange-200 dark:border-orange-500/40 bg-gradient-to-b from-orange-50 to-amber-50 dark:from-orange-500/10 dark:to-amber-500/10"
            const icon =
              position === 1 ? <Trophy className="h-5 w-5 text-yellow-500 dark:text-yellow-400" /> :
              position === 2 ? <Medal className="h-5 w-5 text-slate-400 dark:text-slate-300" /> :
                ...
                <Card className={`h-full overflow-hidden transition-transform duration-200 hover:-translate-y-1 ${emphasis}`}>
                  <CardHeader className="pb-4">
                    ...
                    <div className="flex items-center gap-4 pt-2">
                      <Avatar className={`border-4 ${position === 1 ? "h-24 w-24 border-yellow-300 dark:border-yellow-500/60" : "h-20 w-20 border-white dark:border-border"}`}>
                      ...
                      <div className="min-w-0">
                        ...
                        <CardDescription className="line-clamp-2 text-slate-600 dark:text-muted-foreground">
                  ...
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="rounded-2xl bg-white/70 dark:bg-muted/40 p-3">
                      ...
                      {userProjects.length > 0 ? (
                        <div className="flex flex-wrap gap-2">
                          {userProjects.slice(0, 3).map((project) => (
                            <Badge key={project.id} variant="outline" className="bg-white/80 dark:bg-muted/50">
                              {project.name}
                            </Badge>
                          ))}
                          {userProjects.length > 3 ? (
                            <Badge variant="outline" className="bg-white/80 dark:bg-muted/50">
                              +{userProjects.length - 3}
                            </Badge>
                          ) : null}
                        </div>
